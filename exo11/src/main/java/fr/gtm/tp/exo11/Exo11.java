...
import fr.gtm.tp.exo11.parser.CsvParser;

public class Exo11 implements Runnable{
	
	public static final Logger LOGGER = LoggerFactory.getLogger(Exo11.class);
    ...
    public Exo11(String[] args) {
    	...
    	if (args.length==0) {
    		this.showUsage();
    		throw new IllegalArgumentException("Impossible de lancer l'application sans paramètres (c.f. usage).");
    	}
    	this.path=args[0];
    	if(args.length>1) {
	...
	public void run() {
		...
		LOGGER.info("Données chargées par le parseur: ");
